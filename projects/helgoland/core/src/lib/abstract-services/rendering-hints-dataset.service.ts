...
import {
  DatasetType,
  HelgolandTimeseries,
} from '../api-communication/model/internal/dataset';
import {
  BarRenderingHints,
  LineRenderingHints,
...
export abstract class RenderingHintsDatasetService<
...
> extends DatasetService<T> {
    ...
    return new Promise<boolean>((resolve) => {
      if (this.datasetIds.indexOf(internalId) < 0) {
        if (options) {
          ...
          const params: any = {};
          if (this.translateSrvc) {
            params.locale = this.translateSrvc.currentLang;
          }
  ...
  protected createOptionsOfRenderingHints(timeseries: HelgolandTimeseries): T {
    ...
    if (timeseries.renderingHints) {
      if (
        timeseries.renderingHints.properties &&
        timeseries.renderingHints.properties.color
      ) {
        options.color = timeseries.renderingHints.properties.color;
      }
      switch (timeseries.renderingHints.chartType) {
        case 'line':
          this.handleLineRenderingHints(
            timeseries.renderingHints as LineRenderingHints,
            options,
          );
          break;
        case 'bar':
          this.handleBarRenderingHints(
            timeseries.renderingHints as BarRenderingHints,
            options,
          );
          break;
        default:
  ...
  protected handleBarRenderingHints(
