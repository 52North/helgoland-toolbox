...
export class DatasetApiV3Connector implements HelgolandServiceConnector {
  ...
  getServices(
    ...
    if (clone.expanded === undefined) {
      clone.expanded = true;
    }
    return this.api
      .getServices(url, this.createFilter(clone))
      .pipe(map((res) => res.map((s) => this.createService(s, url, filter))));
  }
  ...
  getService(
    ...
    return this.api
      ...
      .pipe(map((s) => this.createService(s, url, filter)));
  }
  ...
  getCategories(
    ...
    return this.api
      .getCategories(url, this.createFilter(filter))
      .pipe(map((res) => res.map((c) => this.createCategory(c))));
  }
  ...
  getCategory(
    ...
    return this.api
      .getCategory(id, url, this.createFilter(filter))
      .pipe(map((res) => this.createCategory(res)));
  }
  ...
  getOfferings(
    ...
    return this.api
      .getOfferings(url, this.createFilter(filter))
      .pipe(map((res) => res.map((o) => this.createOffering(o))));
  }
  ...
  getOffering(
    ...
    return this.api
      .getOffering(id, url, this.createFilter(filter))
      .pipe(map((res) => this.createOffering(res)));
  }
  ...
  getPhenomena(
    ...
    return this.api
      .getPhenomena(url, this.createFilter(filter))
      .pipe(map((res) => res.map((p) => this.createPhenomenon(p))));
  }
  ...
  getPhenomenon(
    ...
    return this.api
      .getPhenomenon(id, url, this.createFilter(filter))
      .pipe(map((res) => this.createPhenomenon(res)));
  }
  ...
  getProcedures(
    ...
    return this.api
      .getProcedures(url, this.createFilter(filter))
      .pipe(map((res) => res.map((p) => this.createProcedure(p))));
  }
  ...
  getProcedure(
    ...
    return this.api
      .getProcedure(id, url, this.createFilter(filter))
      .pipe(map((res) => this.createProcedure(res)));
  }
  ...
  getFeatures(
    ...
    return this.api
      .getFeatures(url, this.createFilter(filter))
      .pipe(map((res) => res.map((f) => this.createFeature(f))));
  }
  ...
  getFeature(
    ...
    return this.api
      .getFeature(id, url, this.createFilter(filter))
      .pipe(map((res) => this.createFeature(res)));
  }
  ...
  getPlatforms(
    ...
    return this.api
      .getFeatures(url, this.createFilter(filter))
      .pipe(map((res) => res.map((f) => this.createStation(f))));
  }
  ...
  getPlatform(
    ...
    return this.api
      .getFeature(id, url, this.createFilter(filter))
      .pipe(map((res) => this.createStation(res)));
  }
  ...
  getDatasets(
    ...
    return this.api
      ...
      .pipe(map((res) => res.map((ds) => this.createDataset(ds, url))));
  }
  ...
  protected createDataset(ds: ApiV3Dataset, url: string): HelgolandDataset {
    ...
    let firstValue: FirstLastValue | undefined,
      lastValue: FirstLastValue | undefined;
    let category: Parameter | undefined,
      feature: Parameter | undefined,
      offering: Parameter | undefined,
      phenomenon: Parameter | undefined,
      procedure: Parameter | undefined,
      service: Parameter | undefined;
    ...
    if (ds.parameters) {
      ...
      service = {
        id: ds.parameters.service.id,
        label: ds.parameters.service.label,
      };
      platformParam = {
        id: ds.parameters.platform.id,
        label: ds.parameters.platform.label,
    ...
    switch (ds.datasetType) {
      case ApiV3DatasetTypes.Timeseries:
        if (
          ds.observationType === ApiV3ObservationTypes.Simple &&
          (ds.valueType === ApiV3ValueTypes.Quantity ||
            ds.valueType === ApiV3ValueTypes.Count)
        ) {
          let platform: HelgolandPlatform;
          if (ds.feature && ds.feature.geometry) {
            platform = new HelgolandPlatform(
              ds.parameters.platform.id,
              ds.parameters.platform.label,
              [],
              ds.feature.geometry,
            );
          } else {
      ...
      this.api.getDataset(internalId.id, internalId.url, filter).subscribe(
        (res) => {
          ...
          if (dataset instanceof HelgolandTimeseries && idx && idx >= 0) {
            this.api
              .getDatasetExtras(internalId.id, internalId.url, {
                fields: ['renderingHints'],
              })
              .subscribe(
                (extras) => {
  ...
  getDatasetData(
    ...
    if (dataset instanceof HelgolandTimeseries) {
      ...
      if (filter.generalize !== undefined) {
        params.generalize = filter.generalize;
      }
        ...
        return forkJoin(requests).pipe(
          map((e) => {
            const mergedResult = e.reduce((previous, current) => {
              const next: HelgolandTimeseriesData = new HelgolandTimeseriesData(
                previous.values.concat(current.values),
              );
              ...
              for (const key in previous.referenceValues) {
                if (previous.referenceValues.hasOwnProperty(key)) {
                  next.referenceValues[key] = {
                    values: previous.referenceValues[key].values.concat(
                      current.referenceValues[key].values,
                    ),
                  };
                  if (previous.referenceValues[key].valueBeforeTimespan) {
                    next.referenceValues[key].valueBeforeTimespan =
                      previous.referenceValues[key].valueBeforeTimespan;
                  }
                  if (current.referenceValues[key].valueAfterTimespan) {
                    next.referenceValues[key].valueAfterTimespan =
                      current.referenceValues[key].valueAfterTimespan;
                  }
                }
              }
              return next;
    ...
    if (dataset instanceof HelgolandTrajectory) {
      return this.api
        .getDatasetData<LocatedTimeValueEntry>(dataset.id, dataset.url, {
    ...
    if (dataset instanceof HelgolandProfile) {
      return this.api
        .getDatasetData<ProfileDataEntry>(dataset.id, dataset.url, {
          timespan: this.createRequestTimespan(timespan),
          unixTime: true,
        })
  ...
  protected createTrajectoryData(
    res: Data<LocatedTimeValueEntry>,
  ): HelgolandTrajectoryData {
    return new HelgolandTrajectoryData(res.values);
  }

  protected createProfileData(
    res: Data<ProfileDataEntry>,
  ): HelgolandProfileData {
  ...
  protected createFilter(
    ...
    if (filter.service) {
      apiFilter.services = [filter.service];
    }
    if (filter.platform) {
      apiFilter.platforms = [filter.platform];
    }
    if (filter.category) {
    ...
    switch (filter.type) {
      case DatasetType.Timeseries:
        ...
        apiFilter.valueTypes = [
          ApiV3ValueTypes.Quantity,
          ApiV3ValueTypes.Count,
        ];
        break;
      case DatasetType.Trajectory:
  ...
  protected createFeature(feature: ApiV3Feature): Feature {
    ...
    if (feature.properties && feature.properties.domainId) {
      f.domainId = feature.properties.domainId;
    }
    return f;
  }
}
