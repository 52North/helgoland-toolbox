...
describe('StaInsertInterfaceService', () => {
  ...
  it('should be created', () => {
    const service: StaReadInterfaceService = TestBed.inject(
      StaReadInterfaceService,
    );
  ...
  it('should fetch things', () => {
    const read: StaReadInterfaceService = TestBed.inject(
      StaReadInterfaceService,
    );
    const del: StaDeleteInterfaceService = TestBed.inject(
      StaDeleteInterfaceService,
    );
    const insert: StaInsertInterfaceService = TestBed.inject(
      StaInsertInterfaceService,
    );
    // clearAll(read, del);
...
function addCompleteThing(
  insert: StaInsertInterfaceService,
  read: StaReadInterfaceService,
) {
  ...
  insert.insertThing(staUrl, thing).subscribe(
    (insThing) => {
      read
        .getThing(staUrl, insThing['@iot.id']!, {
          $select: { Datastreams: true },
          $expand: { Datastreams: true },
        })
        .subscribe((getThing) => {
...
function clearAll(
  read: StaReadInterfaceService,
  del: StaDeleteInterfaceService,
) {
  read
    .getThings(staUrl, { $select: { id: true } })
    .subscribe((res) =>
      res.value.forEach((thing) =>
        del
          .deleteThing(staUrl, thing['@iot.id']!)
          .subscribe(() => console.log(`delete thing ${thing['@iot.id']}`)),
      ),
    );
  ...
  read
    .getFeaturesOfInterest(staUrl, { $select: { id: true } })
    .subscribe((res) =>
      res.value.forEach((thing) =>
        del
          .deleteFeatureOfInterest(staUrl, thing['@iot.id']!)
          .subscribe(() =>
            console.log(`delete feature of interest ${thing['@iot.id']}`),
          ),
      ),
    );
  ...
  read
    .getLocations(staUrl, { $select: { id: true } })
    .subscribe((res) =>
      res.value.forEach((thing) =>
        del
          .deleteLocation(staUrl, thing['@iot.id']!)
          .subscribe(() => console.log(`delete location ${thing['@iot.id']}`)),
      ),
    );
  ...
  read
    .getObservedProperties(staUrl, { $select: { id: true } })
    .subscribe((res) =>
      res.value.forEach((thing) =>
        del
          .deleteObservedProperty(staUrl, thing['@iot.id']!)
          .subscribe(() =>
            console.log(`delete observed property ${thing['@iot.id']}`),
          ),
      ),
    );
  read
    .getSensors(staUrl, { $select: { id: true } })
    .subscribe((res) =>
      res.value.forEach((thing) =>
        del
          .deleteSensor(staUrl, thing['@iot.id']!)
          .subscribe(() => console.log(`delete sensor ${thing['@iot.id']}`)),
      ),
    );
}
