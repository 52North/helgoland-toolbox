...
export class StaApiV1Connector implements HelgolandServiceConnector {
  ...
  getCategories(
    ...
    return this.sta
      .aggregatePaging(
        this.sta.getObservedProperties(
          url,
  ...
  getCategory(
    ...
    if (this.filterTimeseriesMatchesNot(filter)) {
      return throwError(() => new Error('Could not create category'));
    }
    return this.sta
      .getObservedProperty(url, id)
      .pipe(map((prop) => this.createCategory(prop)));
  }
  ...
  getOfferings(
    ...
    return this.sta
      .aggregatePaging(
        this.sta.getThings(url, this.createOfferingsFilter(filter)),
      )
      .pipe(map((things) => things.value.map((t) => this.createOffering(t))));
  }

  protected createOfferingsFilter(
    params: HelgolandParameterFilter,
  ): StaFilter<ThingSelectParams, ThingExpandParams> {
    if (params) {
  ...
  getOffering(
    ...
    if (this.filterTimeseriesMatchesNot(filter)) {
      return throwError(() => new Error('Could not create offering'));
    }
  ...
  getPhenomena(
    ...
    return this.sta
      .aggregatePaging(this.sta.getObservedProperties(url, staFilter))
      .pipe(
        map((obsProps) => obsProps.value.map((e) => this.createPhenomenon(e))),
      );
  }

  protected createPhenomenaFilter(
    params: HelgolandParameterFilter,
  ): StaFilter<ObservedPropertySelectParams, ObservedPropertyExpandParams> {
    if (params) {
      const filterList: string[] = [];
      if (params.category) {
  ...
  getPhenomenon(
    ...
    if (this.filterTimeseriesMatchesNot(filter)) {
      return throwError(() => new Error('Could not create phenomenon'));
    }
    return this.sta
      .getObservedProperty(url, id)
  ...
  getProcedures(
    ...
    return this.sta
      .aggregatePaging(
        this.sta.getSensors(url, this.createProceduresFilter(filter)),
      )
      .pipe(
        map((sensors) => sensors.value.map((s) => this.createProcedure(s))),
      );
  }

  protected createProceduresFilter(
    params: HelgolandParameterFilter,
  ): StaFilter<SensorSelectParams, SensorExpandParams> {
    if (params) {
      const filterList: string[] = [];
      if (params.category) {
        filterList.push(
  ...
  getProcedure(
    ...
    if (this.filterTimeseriesMatchesNot(filter)) {
      return throwError(() => new Error('Could not create procedure'));
    }
    return this.sta
      .getSensor(url, id)
  ...
  getFeatures(
    ...
    staFilter.$orderby = 'name';
    return this.sta
      .aggregatePaging(this.sta.getLocations(url, staFilter))
      .pipe(map((locs) => locs.value.map((l) => this.createFeature(l))));
  }

  protected createFeaturesFilter(
    params: HelgolandParameterFilter,
  ...
  getFeature(
    ...
    if (this.filterTimeseriesMatchesNot(filter)) {
      return throwError(() => new Error('Could not create feature'));
    }
    return this.sta
      .getLocation(url, id)
      .pipe(map((loc) => this.createFeature(loc)));
  }
  ...
  getPlatforms(
    ...
    return this.sta
      .aggregatePaging(
        this.sta.getLocations(url, this.createStationFilter(filter)),
      )
      .pipe(
  ...
  protected createCategoriesFilter(
    params: HelgolandParameterFilter,
  ): StaFilter<ObservedPropertySelectParams, ObservedPropertyExpandParams> {
    if (params) {
      const filterList: string[] = [];
  ...
  getDatasets(
    ...
    if (filter.expanded) {
      ...
      const firstExpandFilter =
        'Observations($orderby=phenomenonTime desc;$top=1)';
      firstFilter.$expand = firstFilter.$expand
        ? firstFilter.$expand + `,${firstExpandFilter}`
        : firstExpandFilter;
      ...
      const firstRequest = this.sta.aggregatePaging(
        this.sta.getDatastreams(url, firstFilter),
      );
      const lastRequest = this.sta.aggregatePaging(
        this.sta.getDatastreams(url, lastFilter),
      );
      return forkJoin({ first: firstRequest, last: lastRequest }).pipe(
        map((res) => {
          return res.first.value.map((ds) => {
            ...
            const match = res.last.value.find(
              (e) => e['@iot.id'] === ds['@iot.id'],
            );
      ...
      return this.sta
        .aggregatePaging(
          this.sta.getDatastreams(url, this.createDatastreamFilter(filter)),
        )
        .pipe(map((ds) => ds.value.map((d) => this.createTimeseries(d, url))));
    }
  }

  protected getFirstLast(obs: Observation[] | undefined) {
    ...
    if (obs?.length === 1) {
      ...
      if (phenomenonTime && typeof result === 'number') {
        firstLast = new FirstLastValue(
          new Date(phenomenonTime).getTime(),
          result,
        );
      }
    }
  ...
  protected createDatastreamFilter(
    params: DatasetFilter,
  ): StaFilter<DatastreamSelectParams, DatastreamExpandParams> {
  ...
  protected createFirstLastValue(obs: Observation): FirstLastValue | undefined {
    if (obs && obs.phenomenonTime && obs.result) {
      return {
        timestamp: new Date(obs.phenomenonTime).valueOf(),
        value: parseFloat(obs.result),
      };
    }
    return undefined;
  }
  ...
  getDataset(
    ...
    return forkJoin({ first: firstRequest, last: lastRequest }).pipe(
      map((res) => {
        ...
        return this.createExpandedTimeseries(
          res.first,
          first,
          last,
          [],
  ...
  getDatasetData(
    ...
    return this.sta
      .aggregatePaging(
        this.sta.getDatastreamObservationsRelation(dataset.url, dataset.id, {
          $orderby: 'phenomenonTime',
          $filter: this.createTimespanFilter(timespan),
          $top: 200,
        }),
      )
  ...
  protected createTimespanFilter(timespan: Timespan): string {
    ...
    return `phenomenonTime ge ${moment(timespan.from).format(
      format,
    )} and phenomenonTime le ${moment(timespan.to).format(format)}`;
  }
  ...
  protected createTimeseries(ds: Datastream, url: string): HelgolandDataset {
    if (ds['@iot.id'] && ds.Thing) {
      const name = this.createTimeseriesName(
        ds,
  ...
  protected createTsParameter(
    ...
    parameters.additional!['phenomenon_group'] =
  ...
  protected createExpandedTimeseries(
    ds: Datastream,
    first: FirstLastValue | undefined,
    last: FirstLastValue | undefined,
  ...
  protected createTimeseriesName(
    ds: Datastream,
    parameter?: ParameterConstellation,
  ) {
    if (ds.name) {
      return ds.name;
    }
  ...
  protected createPhenomenon(obsProp: ObservedProperty): Phenomenon {
    if (obsProp['@iot.id'] && obsProp.name)
  ...
  protected createServices(
    url: string,
    paramfilter: HelgolandParameterFilter,
  ): Observable<HelgolandService[]> {
  ...
  protected createFilter(
    filterList: string[],
  ): StaFilter<StaSelectParams, StaExpandParams> {
