...
export class StaApiCustomConnector extends StaApiV1Connector {
  ...
  override getPlatform(
    ...
    return this.sta
      .getLocation(url, id, {
        $expand:
          'Things($expand=Locations,Datastreams($filter=properties/hidden ne true;$expand=Thing,Sensor,ObservedProperty))',
      })
    ...
    return forkJoin({ first: firstRequest, last: lastRequest }).pipe(
      mergeMap((res) => {
        ...
        if (res.first.properties?.['referenceDatastreamList']) {
          const refValueIds: string[] =
            res.first.properties?.['referenceDatastreamList'];
          const req = refValueIds.map((id) =>
            this.sta.getDatastream(internalId.url, id, {
              $expand:
                'Thing,Observations($orderby=phenomenonTime desc;$top=1)',
            }),
          );
  ...
  override getDatasetData(
    ...
    const mainDataReq = this.createDatasetDataRequest(
      dataset.id,
      dataset.url,
      timeFilter,
    );
    if (filter.expanded) {
      const requests: {
        [key: string]: Observable<StaValueListResponse<Observation>>;
      } = { main: mainDataReq };
      dataset.referenceValues.forEach((rv) => {
        requests[rv.referenceValueId] = this.createDatasetDataRequest(
          rv.referenceValueId,
          dataset.url,
          timeFilter,
        );
      ...
      return forkJoin(requests).pipe(
        map((res) => {
          ...
          dataset.referenceValues.forEach(
            (e) =>
              (refValues[e.referenceValueId] = {
                values: this.createTimeValueTuple(
                  res[e.referenceValueId].value,
                ),
              }),
          );
          return new HelgolandTimeseriesData(
            this.createTimeValueTuple(res['main'].value),
            refValues,
          );
        }),
      );
    } else {
      return mainDataReq.pipe(
        map(
          (res) =>
            new HelgolandTimeseriesData(this.createTimeValueTuple(res.value)),
        ),
      );
    }
  }

  protected override createTsParameter(
    ds: Datastream,
    thing: Thing,
  ): ParameterConstellation {
  ...
  override getCategories(
    ...
    return this.sta
      .aggregatePaging(this.sta.getDatastreams(url, staFilter))
      .pipe(
        map((res) => {
  ...
  protected override createCategoriesFilter(
  ...
  private createDatasetDataRequest(
    id: string,
    url: string,
    timeFilter: string,
  ) {
    return this.sta.aggregatePaging(
      this.sta.getDatastreamObservationsRelation(url, id, {
  ...
  protected override createPhenomenaFilter(
    params: HelgolandParameterFilter,
  ): StaFilter<ObservedPropertySelectParams, ObservedPropertyExpandParams> {
    if (params) {
      ...
      if (params.category) {
        filterList.push(
          `indexof(Datastreams/properties/tags,'${params.category}') ge 1`,
        );
      }
      if (params.feature) {
        filterList.push(
  ...
  protected override createDatastreamFilter(
  ...
  protected override createProceduresFilter(
    params: HelgolandParameterFilter,
  ...
  protected override createFeaturesFilter(
    params: HelgolandParameterFilter,
    ...
    if (params) {
      ...
      if (params.category) {
        filterList.push(
          `indexof(Things/Datastreams/properties/tags,'${params.category}') ge 1`,
        );
      }
      if (params.phenomenon) {
        filterList.push(
