...
export class DatasetTableComponent
  extends DatasetPresenterComponent<DatasetOptions, any>
  implements OnInit
{
  /*
  ...
  */
  ...
  public sort(event: any) {
    ...
    // set CSS classes
    (Array.from(event.target.parentElement.children) as Element[]).forEach(
    ...
    if (by === 'datetime') {
      sortCallback = (e1: any, e2: any) =>
        directionNumber * (e1.datetime - e2.datetime);
    } else {
      const index = parseInt(by, 10);
      // basically the same as above, but take care of 'undefined' values
      sortCallback = (e1: any, e2: any) =>
        e1.values[index] === undefined
          ? 1
          : e2.values[index] === undefined
            ? -1
  ...
  protected removeDataset(internalId: string) {
    ...
    // @ts-ignore
    // this.preparedData = this.preparedData.filter(
    // (e) => e.values.reduce((a, c) => a || c, undefined) !== undefined,
    // );
  ...
  protected addDataset(id: string, url: string): void {
    ...
    this.servicesConnector
      .getDataset({ id, url }, { type: DatasetType.Timeseries })
  ...
  protected datasetOptionsChanged(
    ...
    if (
      this.timeseriesArray.some(
        (e) => e !== undefined && e.internalId === internalId,
      )
    ) {
      const index = this.getIndexFromInternalId(internalId);
  ...
  private addTimeseries(timeseries: HelgolandTimeseries) {
    this.timeseriesArray[this.getIndexFromInternalId(timeseries.internalId)] =
      timeseries;
  ...
  private loadTsData(timeseries: HelgolandTimeseries) {
    if (this.timespan) {
      ...
      this.servicesConnector
        .getDatasetData(timeseries, this.timespan)
        .subscribe((result) => {
  ...
  private prepareData(
    ...
    if (this.preparedData.length === 0) {
      ...
      while (j < newdata.length) {
        ...
        if (
          this.preparedData[i] &&
          this.preparedData[i].datetime === newdata[j].datetime
        ) {
