  ...
  implements HttpServiceInterceptor
{
  ...
  public intercept(
    ...
    if (!metadata.forceUpdate) {
      ...
      intersectedCache = this.cache.getIntersection(
        req.url,
        reqTimespan,
        generalize,
      );
      ...
      if (
        intersectedCache &&
        (intersectedCache.timespans.length === 0 ||
          Math.floor(intersectedCache.timespans[0].from / 1000) ===
            Math.floor(intersectedCache.timespans[0].to / 1000))
      ) {
        if (intersectedCache.cachedObjects[0]) {
          ...
          return new Observable<HttpEvent<any>>(
            (observer: Observer<HttpEvent<any>>) => {
              const httpResponse =
                intersectedCache.cachedObjects[0].httpResponse;
              const resHttpEvent = this.createHttpResponse(
                urlID,
                expanded,
                httpResponse,
                intersectedCache,
        ...
        shared.subscribe(
          (res) => {
            if (res instanceof HttpResponse) {
              const expirationTime = metadata.expirationAtMs
                ? metadata.expirationAtMs
                : this.expirationAtMs;
              const resultUrl = this.getUrlWithoutParams(res.url);
              // for ts data
              const cachedItem: CachedObject = {
                values: !expanded ? res.body : res.body[urlID],
                expirationDate: moment(moment(new Date()))
                  .add(expirationTime, 'milliseconds')
                  .toDate(),
                expirationAtMs: expirationTime,
                httpResponse: res,
                requestTs: this.decodeTimespan(
                  newRequest.params.get('timespan'),
                ),
              };
              ...
              if (
                !originReq &&
                intersectedCache &&
                intersectedCache.cachedObjects.length > 0
              ) {
                if (cachedItem.values.values.length > 0) {
                  res = this.createHttpResponse(
                    urlID,
                    expanded,
                    res,
                    intersectedCache,
  ...
  private createHttpResponse(
    ...
    intersected: CachedIntersection,
    newCachedObject?: CachedObject,
  ): HttpEvent<any> {
    ...
    if (newCachedObject) {
      ...
      newObjValuesTimespan = new Timespan(
        newCachedObject.values.values[0][0],
        newCachedObject.values.values[
          newCachedObject.values.values.length - 1
        ][0],
      );
      if (newObjValuesTimespan.to <= resObj.values[0][0]) {
        resObj.values = newCachedObject.values.values.concat(resObj.values);
        resObj.referenceValues = this.concatReferenceValues(
          resObj,
          newCachedObject.values,
        );
        resObj.valueBeforeTimespan = this.selectValueBeforeTimespan(
          resObj,
          newCachedObject.values,
        );
        resObj.valueAfterTimespan = this.selectValueAfterTimespan(
          resObj,
          newCachedObject.values,
        );
      }
    ...
    for (let i = 1; i < intersected.cachedObjects.length; i++) {
      ...
      if (
        newCachedObject &&
        newObjValuesTimespan.from >=
          resObj.values[resObj.values.length - 1][0] &&
        newObjValuesTimespan.to <= currVal.values[0][0]
      ) {
      ...
      if (
        i >= intersected.cachedObjects.length - 1 &&
        newCachedObject &&
        newObjValuesTimespan.from >=
          currVal.values[currVal.values.length - 1][0]
      ) {
        resObj.values = resObj.values.concat(newCachedObject.values.values);
        resObj.referenceValues = this.concatReferenceValues(
          newCachedObject.values,
    ...
    if (
      resObj.valueBeforeTimespan &&
      resObj.valueBeforeTimespan[0] > resObj.values[0][0]
    ) {
      resObj.valueBeforeTimespan = resObj.values[0];
    }
    if (
      resObj.valueAfterTimespan &&
      resObj.valueAfterTimespan[0] < resObj.values[resObj.values.length - 1][0]
    ) {
   ...
   */
  private concatReferenceValues(
    base: Data<TimeValueTuple>,
    adds: Data<TimeValueTuple>,
  ): ReferenceValues<TimeValueTuple> {
    const res: ReferenceValues<TimeValueTuple> = Object.assign(
      {},
  ...
  private decodeTimespan(ts: string | null): Timespan | null {
    if (ts) {
      ...
      return new Timespan(
        Math.min(
          moment(new Date(start)).unix() * 1000,
          moment(new Date(end)).unix() * 1000,
        ),
        Math.max(
          moment(new Date(start)).unix() * 1000,
          moment(new Date(end)).unix() * 1000,
   ...
   */
  private selectValueBeforeTimespan(
    el1: Data<TimeValueTuple>,
    el2: Data<TimeValueTuple>,
  ): TimeValueTuple | undefined {
    if (el1.valueBeforeTimespan) {
      if (el2.valueBeforeTimespan) {
        return el1.valueBeforeTimespan[0] < el2.valueBeforeTimespan[0]
          ? el1.valueBeforeTimespan
          : el2.valueBeforeTimespan;
      } else {
   ...
   * @param el2 {Data<TimeValueTuple>}
   */
  private selectValueAfterTimespan(
    el1: Data<TimeValueTuple>,
    el2: Data<TimeValueTuple>,
  ): TimeValueTuple | undefined {
    if (el1.valueAfterTimespan) {
      if (el2.valueAfterTimespan) {
        return el1.valueAfterTimespan[0] < el2.valueAfterTimespan[0]
          ? el1.valueAfterTimespan
          : el2.valueAfterTimespan;
      } else {
