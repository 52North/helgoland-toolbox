...
} from './local-http-cache-interval';
...
describe('LocalHttpCacheInterval', () => {
  ...
  it('should be created', inject(
    [LocalHttpCacheInterval],
    (service: LocalHttpCacheInterval) => {
      expect(service).toBeTruthy();
    },
  ));

  it('should cache', inject(
    [LocalHttpCacheInterval],
    (service: LocalHttpCacheInterval) => {
  ...
  it('should cache for generalized and not generalized data', inject(
    [LocalHttpCacheInterval],
    (service: LocalHttpCacheInterval) => {
      const currTimespanToday = defTsSrvc.getInterval(DefinedTimespan.TODAY);
      const currTimespanYesterday = defTsSrvc.getInterval(
        DefinedTimespan.YESTERDAY,
      );
      const url = 'uniqueID';
      const el: CachedObject = {
        values: {
      ...
      const el2: CachedObject = {
        ...
        expirationDate: moment(new Date()).add(2, 'hours').toDate(),
        expirationAtMs: 3000,
        requestTs: new Timespan(
          currTimespanYesterday.from,
          currTimespanYesterday.to,
        ),
        httpResponse: new HttpResponse(),
      };
  ...
  it('should cache two objects in correct order (ordered by start time)', inject(
    [LocalHttpCacheInterval],
    (service: LocalHttpCacheInterval) => {
  ...
  it('should avoid caching objects with same timespans', inject(
    [LocalHttpCacheInterval],
    (service: LocalHttpCacheInterval) => {
      const currTimespan1 = defTsSrvc.getInterval(DefinedTimespan.TODAY);
      const currTimespan2 = defTsSrvc.getInterval(
        DefinedTimespan.TODAY_YESTERDAY,
      );
      const generalize = false;
      const url1 = 'uniqueID1';
      const el1: CachedObject = {
  ...
  it('should get intersection correctly', inject(
    [LocalHttpCacheInterval],
    (service: LocalHttpCacheInterval) => {
      ...
      const el1: CachedObject = {
        ...
        requestTs: new Timespan(100, 300),
      ...
      const el11: CachedObject = {
        ...
        expirationDate: expirationDate,
        expirationAtMs: expirationAtMs,
        requestTs: new Timespan(301, 350),
        httpResponse: httpEvent,
      };
      const el2: CachedObject = {
        values: {
        ...
        expirationDate: expirationDate,
        expirationAtMs: expirationAtMs,
        requestTs: new Timespan(500, 700),
        httpResponse: httpEvent,
      };
      const el21: CachedObject = {
        ...
        expirationDate: expirationDate,
        expirationAtMs: expirationAtMs,
        requestTs: new Timespan(701, 750),
        httpResponse: httpEvent,
      };
      const el3: CachedObject = {
        ...
        expirationDate: expirationDate,
        expirationAtMs: expirationAtMs,
        requestTs: new Timespan(900, 1100),
        httpResponse: httpEvent,
      };
        ...
        cachedObjects: [
          {
            values: {
              ...
              valueBeforeTimespan: [100, 1],
              valueAfterTimespan: [300, 5],
            },
            expirationDate: expirationDate,
            expirationAtMs: expirationAtMs,
            requestTs: new Timespan(100, 300),
        ...
        cachedObjects: [
          {
            values: {
              ...
              valueBeforeTimespan: [500, 1],
              valueAfterTimespan: [700, 5],
            },
            expirationDate: expirationDate,
            expirationAtMs: expirationAtMs,
            requestTs: new Timespan(500, 700),
      ...
      const result9: CachedIntersection = {
        ...
        timespans: [
          new Timespan(0, 99),
          new Timespan(351, 499),
          new Timespan(751, 899),
          new Timespan(1101, 1400),
        ],
      };
      ...
      const elURL2: CachedObject = {
        ...
        expirationDate: expirationDate,
        expirationAtMs: expirationAtMs,
        requestTs: new Timespan(100, 100),
        httpResponse: httpEvent,
      };
      ...
      expect(service.get(url1, generalize)).toEqual([
        el1,
        el11,
        el2,
        el21,
      ...
      // should be: complete intersection with 1 element of 1 timeperiod
      expect(
        service.getIntersection(url2, new Timespan(100, 100), generalize),
      ).toEqual(resultURL2);
      // should be: outside range
      expect(
        service.getIntersection(url1, new Timespan(0, 50), generalize),
      ).toEqual(result1);
      // should be: intersection by 1 timeperiod before first cached item
      expect(
        service.getIntersection(url1, new Timespan(0, 100), generalize),
      ).toEqual(result2);
      // should be: intersection by 1 timeperiod after first cached item
      expect(
        service.getIntersection(url1, new Timespan(1100, 1300), generalize),
      ).toEqual(result3);
      // should be: no difference = complete intersection
      expect(
        service.getIntersection(url1, new Timespan(101, 299), generalize),
      ).toEqual(result4);
      // should be: no difference = complete intersection
      expect(
        service.getIntersection(url1, new Timespan(100, 300), generalize),
      ).toEqual(result5);
      // should be: no difference = complete intersection
      expect(
        service.getIntersection(url1, new Timespan(100, 301), generalize),
      ).toEqual(result51);
      // should be: no intersection by 1 timeperiod before first cached item, intersection inbetween two elements
      expect(
        service.getIntersection(url1, new Timespan(99, 301), generalize),
      ).toEqual(result52);
      // should be: no difference = complete intersection
      expect(
        service.getIntersection(url1, new Timespan(550, 650), generalize),
      ).toEqual(result6);
      // should be: intersection inbetween two timeperiods
      expect(
        service.getIntersection(url1, new Timespan(200, 600), generalize),
      ).toEqual(result7);
      // should be: intersection inbetween four timeperiods
      expect(
        service.getIntersection(url1, new Timespan(200, 800), generalize),
      ).toEqual(result8);
      // should be: intersection inbetween three timeperiods
      expect(
        service.getIntersection(url1, new Timespan(310, 710), generalize),
      ).toEqual(result81);
      // should be: intersection in five timeperiods with outside range
      expect(
        service.getIntersection(url1, new Timespan(0, 1400), generalize),
      ).toEqual(result9);
      // should be: intersection in four timeperiods, endings inside intersection
      expect(
        service.getIntersection(url1, new Timespan(300, 950), generalize),
      ).toEqual(result10);
      // should be: complete intersection with element of 1 timeperiod
      expect(
        service.getIntersection(url1, new Timespan(1100, 1100), generalize),
      ).toEqual(result11);
    },
  ));
});
