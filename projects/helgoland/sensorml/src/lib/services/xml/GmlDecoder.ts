...
export class GmlDecoder {
  ...
  public decodeTimeInstant(elem: Element): ReturnObject<AbstractTime> {
    ...
    if (timeElem != null) {
      ...
      const timePositionElem = this.utils.getElement(
        timeElem,
        'timePosition',
        NAMESPACES.GML,
      );
  ...
  public decodeTimePeriod(elem: Element): ReturnObject<AbstractTime> {
    ...
    if (timeElem != null) {
      ...
      const beginPositionElem = this.utils.getElement(
        timeElem,
        'beginPosition',
        NAMESPACES.GML,
      );
      ...
      const endPositionElem = this.utils.getElement(
        timeElem,
        'endPosition',
        NAMESPACES.GML,
      );
  ...
  public decodeAbstractGML(elem: Element, object: AbstractGML): void {
    ...
    const descriptionReferenceElem = this.utils.getElement(
      elem,
    ...
    const identifierElem = this.utils.getElement(
      elem,
      'identifier',
      NAMESPACES.GML,
    );
    ...
    object.name = this.utils.getDecodedList(
      ...
      NAMESPACES.GML,
      this._profileIDMap,
      (nameElem) => this.decodeCodeType(nameElem),
    );
  }
  ...
  public decodeEnvelope(elem: Element): Envelope {
    ...
    if (envelopeElem != null) {
      ...
      const lowerCorner = this.utils.getElement(
        envelopeElem,
        'lowerCorner',
        NAMESPACES.GML,
      );
      ...
      const upperCorner = this.utils.getElement(
        envelopeElem,
        'upperCorner',
        NAMESPACES.GML,
      );
  ...
  public decodeReferenced(elem: Element, referenced: Referenced): void {
    ...
    if (
      elem.hasAttribute('srsDimension') &&
      !isNaN(+elem.getAttribute('srsDimension'))
    ) {
  ...
  private getTime(elem: Element): Date {
    if (
      elem.hasAttribute('indeterminatePosition') &&
      elem.getAttribute('indeterminatePosition') === 'unknown'
    ) {
      return null;
    } else {
