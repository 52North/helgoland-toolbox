import { Component, PhysicalSystem } from '../../model/sml';
...
describe('SensorMLDocumentEncoder', () => {
  ...
  it('should serialize the component', () => {
    ...
    expect(
      doc.eval(
        '/sml:PhysicalSystem/sml:components/sml:ComponentList/sml:component/@name',
      )[0].value,
    ).toBe(ps.components.components[0].name);
    expect(
      doc.eval(
        '/sml:PhysicalSystem/sml:components/sml:ComponentList/sml:component/@href',
      )[0].value,
    ).toBe(ps.components.components[0].href);
    expect(
      doc.eval(
        '/sml:PhysicalSystem/sml:components/sml:ComponentList/sml:component/@xlink:href',
      )[0].value,
    ).toBe(ps.components.components[0].href);
  });
});
