// @ts-nocheck
import { AbstractTime } from '../../model/gml';
import {
  AbstractAlgorithm,
...
export class SensorMLDecoder {
  ...
  private createProcessOfElement(element: Element): AbstractProcess {
    ...
    if (
      this.utils.getElement(element, AggregateProcess.NAME(), NAMESPACES.SML)
    ) {
      return new AggregateProcess();
    }
    if (
      this.utils.getElement(element, PhysicalComponent.NAME(), NAMESPACES.SML)
    ) {
  ...
  public decodeSpatialFrame(elem: Element): ReturnObject<SpatialFrame> {
    ...
    if (spatialFrameElem != null) {
      this.sweDecoder.decodeAbstractSweIdentifiable(
        spatialFrameElem,
        spatialFrame,
      );

      const originElem = this.utils.getElement(
        spatialFrameElem,
        'origin',
        NAMESPACES.SML,
      );
  ...
  public decodeAggregatingProcess(
    ...
    if (componentsElem != null) {
      const returnObject: ReturnObject<ComponentList> =
        this.decodeComponentList(componentsElem);
      if (returnObject) {
    ...
    const connectionsElem = this.utils.getElement(
      elem,
      'connections',
      NAMESPACES.SML,
    );
    if (connectionsElem != null) {
      const returnObject: ReturnObject<ConnectionList> =
        this.decodeConnectionList(connectionsElem);
      if (returnObject) {
  ...
  public decodeAbstractProcess(elem: Element, object: AbstractProcess): void {
    ...
    const settings = this.utils.getElement(
      elem,
      'configuration',
      NAMESPACES.SML,
    );
    ...
    const features = this.utils.getElement(
      elem,
      'featuresOfInterest',
      NAMESPACES.SML,
    );
    if (features != null) {
      const featureListRo: ReturnObject<FeatureList> =
        this.decodeFeatureList(features);
      if (featureListRo) {
        object.featureOfInterest = featureListRo.value;
        this._profileIDMap = this.utils.processProfileID(
          featureListRo.docElement,
          object,
          'featureOfInterest',
          this._profileIDMap,
        );
    ...
    if (inputsElem != null) {
      const inputListRo: ReturnObject<InputList> =
        this.decodeInputList(inputsElem);
      if (inputListRo) {
        object.inputs = inputListRo.value;
        this._profileIDMap = this.utils.processProfileID(
          inputListRo.docElement,
          object,
          'inputs',
          this._profileIDMap,
        );
    ...
    if (outputsElem != null) {
      const outputListRo: ReturnObject<OutputList> =
        this.decodeOutputList(outputsElem);
      if (outputListRo) {
        object.outputs = outputListRo.value;
        this._profileIDMap = this.utils.processProfileID(
          outputListRo.docElement,
          object,
          'outputs',
          this._profileIDMap,
        );
    ...
    const parameters = this.utils.getElement(
      elem,
      'parameters',
      NAMESPACES.SML,
    );
    if (parameters != null) {
      const parameterListRo: ReturnObject<ParameterList> =
        this.decodeParameterList(parameters);
      if (parameterListRo) {
        object.parameters = parameterListRo.value;
        this._profileIDMap = this.utils.processProfileID(
          parameterListRo.docElement,
          object,
          'parameters',
          this._profileIDMap,
        );
  ...
  public decodeFeatureList(elem: Element): ReturnObject<FeatureList> {
    ...
    if (featureListElem != null) {
      ...
      featureList.feature = this.utils.getDecodedList<FeatureProperty>(
        featureListElem,
        'feature',
        NAMESPACES.SML,
        this._profileIDMap,
        (feature) => {
          ...
          this.gmlDecoder.decodeAssociationAttributeGroup(
            feature,
            featureProperty,
          );
  ...
  public decodeMode(elem: Element): ReturnObject<Mode> {
    ...
    if (modeElem != null) {
      ...
      if (settingsElem != null) {
        const returnObject: ReturnObject<Settings> =
          this.decodeSettings(settingsElem);
        if (returnObject) {
  ...
  public decodeDescribedObject(
    ...
    const secConstElem = this.utils.getElement(
      elem,
      'securityConstraints',
      NAMESPACES.SML,
    );
  ...
  public decodeConnection(elem: Element): ReturnObject<Connection> {
    ...
    if (connectionElem != null) {
      ...
      returnObject = this.utils.getAttributeOfElement(
        connectionElem,
        'destination',
        NAMESPACES.SML,
        'ref',
  ...
  public decodeInputOrOutputOrParameter(
    ...
    if (elem.firstElementChild != null) {
      const returnObject: ReturnObject<ObservableProperty> =
        this.decodeObservableProperty(elem);
      ...
      const returnObject1: ReturnObject<DataInterface> =
        this.decodeDataInterface(elem);
      if (returnObject1) {
        const dataInterface = returnObject1.value;
        if (dataInterface != null) {
          param.value = dataInterface;
          this._profileIDMap = this.utils.processProfileID(
            returnObject1.docElement,
            param,
            'value',
            this._profileIDMap,
          );
      ...
      const returnObject2: ReturnObject<AbstractDataComponent> =
        this.sweDecoder.decodeDataComponent(elem);
      if (returnObject2) {
        const dataComponent = returnObject2.value;
        if (dataComponent != null) {
          param.value = dataComponent;
          this._profileIDMap = this.utils.processProfileID(
            returnObject2.docElement,
            param,
            'value',
            this._profileIDMap,
          );
  ...
  public decodeDataInterface(elem: Element): ReturnObject<DataInterface> {
    ...
    if (dataInterfaceElem != null) {
      ...
      const dataElem = this.utils.getElement(
        dataInterfaceElem,
        'data',
        NAMESPACES.SML,
      );
      if (dataElem != null) {
        const returnObject: ReturnObject<SweDataStream> =
          this.sweDecoder.decodeDataStream(dataElem);
        if (returnObject) {
      ...
      const interfaceParams = this.utils.getElement(
        dataInterfaceElem,
        'interfaceParameters',
        NAMESPACES.SML,
      );
      if (interfaceParams != null) {
        const returnObject: ReturnObject<SweDataRecord> =
          this.sweDecoder.decodeDataRecord(interfaceParams);
        if (returnObject) {
  ...
  public decodeKeywordList(elem: Element): ReturnObject<KeywordList> {
    ...
    if (keywordListElem != null) {
      ...
      this.utils.getDecodedList(
        ...
        (keywords) =>
          new ReturnObject(
            keywordList.keywords.push(keywords.textContent),
            null,
          ),
      );
  ...
  public decodeProcessMethodProcess(
    ...
    if (methodElem != null) {
      const returnObject: ReturnObject<ProcessMethod> =
        this.decodeProcessMethod(methodElem);
      if (returnObject) {
  ...
  public decodeEvent(elem: Element): ReturnObject<Event> {
    ...
    if (eventElem != null) {
      ...
      event.classification = this.utils.getDecodedList(
        eventElem,
        'classification',
        NAMESPACES.SML,
        this._profileIDMap,
      ...
      if (timeElem != null) {
        const returnObject: ReturnObject<AbstractTime> =
          this.gmlDecoder.decodeTime(timeElem);
        if (returnObject) {
      ...
      const propertiesElem = this.utils.getElement(
        eventElem,
        'property',
        NAMESPACES.SML,
      );
      ...
      const configurationElem = this.utils.getElement(
        eventElem,
        'configuration',
        NAMESPACES.SML,
      );
      if (configurationElem != null) {
        const returnObject: ReturnObject<Settings> =
          this.decodeSettings(configurationElem);
        if (returnObject) {
  ...
  public decodeSetArrayValue(elem: Element): ReturnObject<ArrayValueSetting> {
    ...
    const encodingElem = this.utils.getElement(
      elem,
      'encoding',
      NAMESPACES.SML,
    );
    if (encodingElem != null) {
      const returnObject: ReturnObject<SweEncoding> =
        this.sweDecoder.decodeAbstractEncoding(encodingElem);
      if (returnObject) {
