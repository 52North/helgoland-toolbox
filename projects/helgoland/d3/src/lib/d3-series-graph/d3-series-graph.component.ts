  ...
  implements OnDestroy, AfterViewInit, DoCheck, OnInit, D3GraphInterface
{
    ...
    if (visualMin === undefined || visualMax === undefined) {
      ...
      if (visualMin === undefined) {
        visualMin = d3.min(
          filterUndefined([
            baseDataExtent[0],
            ...dataExtentChildValues.map((e) => e[0]),
          ]),
        );
      }
      ...
      if (visualMax === undefined) {
        visualMax = d3.max(
          filterUndefined([
            baseDataExtent[1],
            ...dataExtentChildValues.map((e) => e[1]),
          ]),
        );
      }
    }
  ...
  protected redrawGraph(): void {
    ...
    this.datasets.forEach((entry) => {
      ...
      if (idx < 0) {
        this.listOfUoms.push(entry.description.uom);
      }
    });
        ...
        if (axisSettings.entry.yAxis.separate) {
          ...
          if (axis) {
            ...
            this.yAxes.push({
              uom: axisSettings.entry.description.uom,
              range: {
                min: axisSettings.visualMin,
                max: axisSettings.visualMax,
              },
              fixedMin: axisSettings.fixedMin,
              fixedMax: axisSettings.fixedMax,
              seperate: false,
  ...
  private drawYaxis(axis: YAxis) {
    const showAxis =
      this.plotOptions.yaxis === undefined ? true : this.plotOptions.yaxis;
    ...
    if (showAxis && axisElemNode) {
      ...
      const text = this.graph
        ...
        .call(
          ...
          axisHeight - 10,
          diagramHeight / 2,
          this.plotOptions.yaxisModifier,
          axis.label,
        );
  ...
  private drawBackground() {
    this.background = this.graph
      .insert<SVGGElement>('svg:rect', ':first-child')
      .attr('width', this.width - this.leftOffset)
      .attr('height', this.height)
    ...
    if (data && data.length > 0 && entry.visible) {
      ...
      if (yaxis) {
        ...
        switch (entry.style.constructor) {
          case BarStyle:
            this.drawBarChart(
              entry as SeriesGraphDataset<BarStyle>,
              idx,
              yaxis.yScale!,
            );
            break;
  ...
  private drawLineChart(
    ds: SeriesGraphDataset<LineStyle>,
    idx: number,
    yScaleBase: d3.ScaleLinear<number, number>,
  ) {
    ...
    if (ds.style.pointSymbol) {
      this.graphBody
        ...
        .attr(
          'transform',
          (de: GraphDataEntry, idx: number, data: any) =>
            `translate(${line.x()(de, idx, data)},${line.y()(de, idx, data)})`,
        )
        .attr('stroke', ds.style.pointBorderColor)
        .attr('fill', ds.style.baseColor)
        .attr(
          'd',
          this.pointSymbolDrawer.getSymbolPath(
  ...
  private drawBarChart(
    ds: SeriesGraphDataset<BarStyle>,
    idx: number,
    yScaleBase: d3.ScaleLinear<number, number>,
  ) {
  ...
  private calculatePointRadius(ds: SeriesGraphDataset<LineStyle>) {
    if (ds.selected) {
      return ds.style.pointRadius > 0
        ? ds.style.pointRadius + this.addLineWidth
