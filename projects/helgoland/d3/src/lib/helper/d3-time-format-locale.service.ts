...
export class D3TimeFormatLocaleService {
  ...
  public formatTime(time: number): string {
    ...
    const format =
      this.roundSecond(time) < curr
        ? this.formatMillisecond
        : this.roundMinute(time) < curr
          ? this.formatSecond
          : this.roundHour(time) < curr
            ? this.formatMinute
            : this.roundDay(time) < curr
              ? this.formatHour
              : this.roundMonth(time) < curr
                ? this.roundWeek(time) < curr
                  ? this.formatDay
                  : this.formatWeek
                : this.roundYear(time) < curr
                  ? this.formatMonth
                  : this.formatYear;
