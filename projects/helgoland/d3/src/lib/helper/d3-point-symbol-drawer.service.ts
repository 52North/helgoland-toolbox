...
export class D3PointSymbolDrawerService {
  ...
  drawSymboleLine(
    ...
    if (entry.options.pointSymbol) {
      const symbolPath = this.getSymbolPath(
        entry.options.pointSymbol,
      ...
      if (symbolPath) {
        drawPane
          ...
          .attr(
            'transform',
            (d) => `translate(${d.xDiagCoord},${d.yDiagCoord})`,
          )
  ...
  private getSymbolPath(
    ...
    if (symbolType) {
      return d3
        ...
        .type(symbolType)
        .size(
          this.calculateSymbolSize(pointSymbol, selected, additionalSize),
        )();
    }
    return null;
  }
