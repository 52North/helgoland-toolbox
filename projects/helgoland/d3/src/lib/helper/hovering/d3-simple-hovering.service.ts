...
export class D3SimpleHoveringService extends D3HoveringService {
  ...
  public showTooltip(elements: HoveringElement[], position: HoverPosition) {
    if (this.anchorElem) {
      ...
      elements.forEach((elem) => {
        const rect: d3.Selection<SVGGElement, any, any, any> =
          this.tooltipContainer!.append('svg:rect');
        rect
          .attr('class', 'mouseHoverDotRect')
          .style('fill', 'white')
          .style('fill-opacity', 1)
  ...
  protected positionTooltip(
    ...
    rect: d3.Selection<SVGGElement, any, any, any>,
    onLeftSide: boolean,
    itemCounter: number,
  ) {
  ...
  protected calculatePointRadius(entry: SeriesGraphDataset<LineStyle>) {
    if (entry.selected) {
      return entry.style.pointRadius > 0
        ? entry.style.pointRadius + this.addLineWidth
        : entry.style.pointRadius;
    } else {
