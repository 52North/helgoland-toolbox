  ...
  implements OnChanges
{
  ...
  protected drawOverviewSelection() {
    ...
    const interval: [number, number] = this.getXDomainByTimestamp(
      this.completeTimespan,
    ...
    const brush = d3
      ...
      .extent([
        [0, 0],
        [this.graphExtent.width, this.graphExtent.height],
      ])
      .on('end', (event) => {
   ...
   */
  protected getXDomainByTimestamp(
    timespan: Timespan,
    width: number,
  ): [number, number] {
    /**
    ...
    const minCalcBrush: number =
      divOverviewTimeWidth * (minDiagramTimestamp - minOverviewTimeInterval);
    const maxCalcBrush: number =
      divOverviewTimeWidth * (maxDiagramTimestamp - minOverviewTimeInterval);
  ...
  protected getTimestampByCoord(
    ...
    const diffOverviewTimeInterval =
      maxOverviewTimeInterval - minOverviewTimeInterval;
    const minDiagramTimestamp: number =
      (minCalcBrush / width) * diffOverviewTimeInterval +
      minOverviewTimeInterval;
    const maxDiagramTimestamp: number =
      (maxCalcBrush / width) * diffOverviewTimeInterval +
      minOverviewTimeInterval;
