...
import { ModalEditTimeseriesOptionsComponent } from '../../components/modal-edit-timeseries-options/modal-edit-timeseries-options.component';
...
export class DiagramViewComponent implements OnInit {
  ...
  jumpToDate(date: Date) {
    this.timeseries.timespan = this.time.centerTimespan(
      this.timeseries.timespan,
      date,
    );
  }
  ...
  selectTimeseries(selected: boolean, internalId: string) {
    if (selected) {
      ...
      this.selectedIds.splice(
        this.selectedIds.findIndex((entry) => entry === internalId),
        1,
      );
    }
  }
  ...
  editOption(options: DatasetOptions) {
    ...
    dialogRef
      .afterClosed()
      .subscribe((_) =>
        this.timeseries.updateDatasetOptions(options, options.internalId),
      );
  }
