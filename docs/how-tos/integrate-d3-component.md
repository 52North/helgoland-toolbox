...
By manually adapting the configuration in `tsconfig.app.js` this issue should be solved. Add to the types-array the type `geojson` which then should look like this:
...
Now we will integrate the directive of the D3 timeseries component. Implement the directive of the graph by adding the following code to `app.component.html`. All fields in the `n52-d3-timeseries-graph`-directive are required to provide options, datasets and other information.
...
Implement the functionality of the `n52-d3-timeseries-graph`-directive by adding the following code to `app.component.ts`:
